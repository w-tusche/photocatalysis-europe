...
def plot_energy_balance_timeseries(
    ...
    """
    Create energy balance time series plot with positive/negative stacked
    areas.
    """
    if time is not None:
        df = df.loc[time]
    ...
    fn = f"ts-balance-{ylabel.replace(' ', '_')}-{resample}_PC_0.pdf"
...
if __name__ == "__main__":
    ...
    n = pypsa.Network(
        "results/raw/150_lv1.25_I_H_2045_3H_PC_925Euro/elec_s_150_lv1.25__I-H_2045.nc"
    )
    with open(r"src/plotting.default.yaml") as stream:
        loaded_conf = yaml.safe_load(stream)
    config = loaded_conf["plotting"]["balance_timeseries"]
    plotting_conf = loaded_conf["plotting"]
    # config = snakemake.params.plotting["balance_timeseries"]
    output_dir = "./img/time_balance/"
    os.makedirs(output_dir, exist_ok=True)

    # Get month ranges for plotting
    # snapshots_from_config = {"start": '2013-01-01', "end": '2014-01-01', "inclusive": "left"}
    # sns = snakemake.params.snapshots
    sns = {"start": "2013-01-01", "end": "2014-01-01", "inclusive": "left"}
    ...
    n.carriers.update({"color": plotting_conf["tech_colors"]})
